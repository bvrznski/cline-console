...
Status is not exported by the activation API. The singleton service and companion therefore inspect Cline's persisted `taskHistory.json`, select the newest entry whose `cwdOnTaskInitialization` exactly matches the workspace, and inspect that task's final `ui_messages.json` record. `completion_result` is terminal; other current UI records represent a running or waiting task. Timestamped session metadata remains a fallback. This avoids both stale `idle` session files and stale bridge `submitted` markers. The Cline task-history identifier is reported as the task ID.
