...
export function parseRequest(line: string): IpcRequest {
  ...
  if (!(["newTask", "reloadTask", "skipWaitingTask", "sendMessage", "cancelTask", "status", "capabilities", "enqueueTasks", "enqueueMessages", "replaceQueue", "clearQueue", "clearWorkspace", "popQueue", "pauseQueue", "resumeQueue", "queueStatus", "activity"] as unknown[]).includes(request.action)) throw new ClineConsoleError("INVALID_ACTION", "Unknown action.");
